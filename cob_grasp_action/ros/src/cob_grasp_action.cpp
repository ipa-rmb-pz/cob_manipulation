...
#include <cob_grasp_action/cob_grasp_action.h>
...
void CobGraspActionServer::initialize()
{
	...
	grasp_server_.reset(new actionlib::SimpleActionServer<ipa_manipulation_msgs::GraspPoseAction>(nh_, GRASPS_GENERATION_OR_ACTION_NAME, boost::bind(&CobGraspActionServer::grasp_generation_cb, this, _1), false));
	grasp_server_->start();
	ROS_INFO("Action client %s started ", GRASPS_GENERATION_OR_ACTION_NAME.c_str());
}

void CobGraspActionServer::run()
{
	ROS_INFO("cob_pick_action...spinning");
	ros::spin();
}

void CobGraspActionServer::grasp_generation_cb(const ipa_manipulation_msgs::GraspPoseGoalConstPtr &goal)
{
...
void CobGraspActionServer::insertObject(std::string object_name, unsigned int object_class, geometry_msgs::PoseStamped object_pose)
{
...
void CobGraspActionServer::fillAllGraspsKIT(unsigned int objectClassId, std::string gripper_type, geometry_msgs::PoseStamped object_pose, std::vector<moveit_msgs::Grasp> &grasps)
{
	grasps.clear();
...
void CobGraspActionServer::fillSingleGraspKIT(unsigned int objectClassId, std::string gripper_type, unsigned int grasp_id, geometry_msgs::PoseStamped object_pose, std::vector<moveit_msgs::Grasp> &grasps)
{
	grasps.clear();
	Grasp *current_grasp = NULL;
...
void CobGraspActionServer::convertGraspKIT(Grasp* current_grasp, geometry_msgs::PoseStamped object_pose, std::vector<moveit_msgs::Grasp> &grasps)
{
	bool debug = true;
	moveit_msgs::Grasp g;
...
void CobGraspActionServer::fillGraspsOR(unsigned int objectClassId, std::string gripper_type, std::string gripper_side, unsigned int grasp_id, geometry_msgs::PoseStamped object_pose, std::vector<moveit_msgs::Grasp> &grasps)
{
...
trajectory_msgs::JointTrajectory CobGraspActionServer::MapHandConfiguration(sensor_msgs::JointState table_config)
{
...
tf::Transform CobGraspActionServer::transformPose(tf::Transform transform_O_from_SDH, tf::Transform transform_HEADER_from_O, std::string object_frame_id)
{
	bool debug = false;
...
moveit_msgs::GripperTranslation CobGraspActionServer::calculateApproachDirection(geometry_msgs::Pose msg_pose_grasp_FOOTPRINT_from_ARM7, geometry_msgs::Pose msg_pose_pre_FOOTPRINT_from_ARM7)
{
...
int main(int argc, char **argv)
{
	...
	CobGraspActionServer *cob_pick_place_action_server = new CobGraspActionServer(std::string(argv[1]));
